...
namespace Sim.Missions {
    /// <summary>
    /// Singleton driving the mission highlight system. Listens to MissionClientManager
    /// events and recomputes which world objects should be outlined based on the
    /// CURRENT STEP of every active mission — not on a MissionPoint id.
    ///
    /// Ciblage hybride :
    ...
    public class MissionHighlightManager : MonoBehaviour {
        ...
        private readonly HashSet<string>       _activeCareers  = new HashSet<string>();
        ...
        public static void Register(MissionHighlightKind kind, string id, MissionHighlightEffect effect,
            ...
            _entries.Add(new Entry { effect = effect, kind = kind, id = id, requiredProfessionId = requiredProfessionId, phase = phase });
        ...
        private void Subscribe() {
            ...
            if (c == null) {
                Debug.LogWarning("[MissionHighlightManager] MissionClientManager not found — highlights won't work.");
                return;
            }
            c.MissionOffered      += OnMissionUpdate;
            c.MissionStepAdvanced += OnMissionUpdate;
            c.MissionFinished     += OnMissionUpdate;
        }
        ...
        private void OnMissionUpdate(MissionClientState _) => RefreshHighlight();
        ...
        private void RefreshHighlight() {
            ...
            if (states != null) {
                foreach (var state in states.Values) {
                    if (state == null || state.Status != MissionStatus.Active) continue;
                    ...
                    if (!string.IsNullOrEmpty(def.ProfessionId)) _activeCareers.Add(def.ProfessionId);
                }
            }
        ...
        private bool ShouldShow(Entry e, MissionHighlightKind activeKinds) {
            ...
            if (string.IsNullOrEmpty(e.requiredProfessionId)) return true;
            return _activeCareers.Contains(e.requiredProfessionId);
        }
