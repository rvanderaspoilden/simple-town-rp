...
namespace Sim.Jobs {
    ...
    /// <see cref="JobBoardClient"/> dès que le joueur local est employé dans cette
    /// catégorie (career gate serveur inchangé). Se redessine sur chaque push serveur.
    /// </summary>
    [RequireComponent(typeof(JobBoard))]
    public class JobBoardDisplay : MonoBehaviour {
        ...
        private JobBoard _board;
        private readonly List<JobBoardCardView> _cards = new List<JobBoardCardView>();
        private bool _subscribed;

        private void Awake() {
            _board = GetComponent<JobBoard>();
        }
        ...
        private void OnDisable() {
            ...
            if (_subscribed && NetworkClient.isConnected) {
                JobBoardClient.Instance.RequestClose(_board.Category);
            }
            _subscribed = false;
        }
        ...
        private void OnBoardUpdated(JobCategory category, JobBoardEntry[] entries) {
            if (category != _board.Category) return;
            Render();
        }
        ...
        private bool LocalEmployedHere {
            get {
                ...
                return data != null && data.CurrentJobCategory == _board.Category;
            }
        }
        ...
        private void EvaluateSubscription() {
            ...
            if (LocalEmployedHere && !_subscribed) {
                JobBoardClient.Instance.RequestOpen(_board.Category);
                _subscribed = true;
        ...
        private void Render() {
            ...
            JobBoardEntry[] entries = JobBoardClient.Instance.GetEntries(_board.Category);
            if (entries == null || entries.Length == 0) return;

            int count = Mathf.Min(entries.Length, maxCards);
            for (int i = 0; i < count; i++) {
                JobBoardEntry e = entries[i];
                JobDefinition def = JobDatabase.GetById(e.jobId);
                string title = def != null && !string.IsNullOrEmpty(def.DisplayNameKey)
                    ? def.DisplayNameKey
                    : e.jobId;

                bool available = e.Status == JobStatus.Available;
                JobBoardCardView card = JobBoardCardView.Create(cardsContainer, cardFontSize);
        ...
        private void GameLogger_LogMissingContainer() {
            ...
            Debug.LogWarning($"[JobBoardDisplay] '{name}' : cardsContainer non assigné — " +
