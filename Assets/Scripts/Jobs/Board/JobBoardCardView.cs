...
namespace Sim.Jobs {
    ...
    ///
    /// Construite intégralement au runtime par <see cref="JobBoardDisplay"/> (pas de
    /// prefab à câbler). Non interactive : aucun raycast UI, ne bloque pas le clic
    /// physique sur le board.
    /// </summary>
    public class JobBoardCardView : MonoBehaviour {
        private Image _background;
        private TextMeshProUGUI _title;

        public static JobBoardCardView Create(Transform parent, int titleFontSize) {
            var go = new GameObject("JobCard", typeof(RectTransform), typeof(Image), typeof(JobBoardCardView));
            go.transform.SetParent(parent, false);

            var view = go.GetComponent<JobBoardCardView>();
