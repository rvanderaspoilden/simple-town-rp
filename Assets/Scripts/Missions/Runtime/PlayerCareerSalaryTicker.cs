...
namespace Sim.Missions {
    ...
    ///   - their profession's `baseSalary` if they have a current profession, or
    ///   - the city's `unemployed_income` otherwise (allocation chômage).
    /// POC behaviour: online-only — disconnected players accrue nothing.
    ///
    /// Owned by MissionSystemBootstrap (created on OnServerStart, destroyed on
    /// OnServerStop).
    /// </summary>
    public class PlayerCareerSalaryTicker : MonoBehaviour {
        ...
        private static void PayAllPlayers() {
            ...
            foreach (var conn in NetworkServer.connections.Values) {
                ...
                var profession = player.CharacterData.CurrentProfession;
                int amount;
                string label;
                if (profession != null) {
                    amount = profession.baseSalary;
                    label = $"Salaire {profession.displayName}";
                } else {
                ...
                GameLogger.System.Debug("CareerPayout {NetId} {Profession} {Amount}",
                    conn.identity.netId, profession != null ? profession.id : "none", amount);
            }
        }
    }
}
