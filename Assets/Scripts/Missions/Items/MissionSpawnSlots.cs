...
namespace Sim.Missions {
    ...
    /// SortingBin / MissionPoint. La scène (City.unity) étant chargée côté serveur,
    /// ce composant existe aussi serveur-side : c'est le serveur qui lit les slots
    /// au spawn des items.
    /// </summary>
    public class MissionSpawnSlots : MonoBehaviour {
        ...
        private static readonly Dictionary<string, MissionSpawnSlots> _all =
            new Dictionary<string, MissionSpawnSlots>();
        ...
        private void Awake() {
            if (string.IsNullOrEmpty(slotsId)) {
                Debug.LogError($"[MissionSpawnSlots] '{name}' has empty slotsId — fix it in the Inspector.");
                return;
            }
            if (_all.ContainsKey(slotsId))
                Debug.LogWarning($"[MissionSpawnSlots] duplicate slotsId '{slotsId}' on '{name}'.");
            _all[slotsId] = this;
        }
        ...
        public static MissionSpawnSlots Get(string id) {
