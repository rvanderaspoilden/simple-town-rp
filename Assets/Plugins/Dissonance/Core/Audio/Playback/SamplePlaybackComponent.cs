...
namespace Dissonance.Audio.Playback
{
        ...
        : MonoBehaviour
    {
        ...
        internal static bool Filter(SpeechSession session, [NotNull] float[] output, int channels, [NotNull] float[] temp, [CanBeNull] AudioFileWriter diagnosticOutput, IAudioOutputSubscriber[] subscribers, out float arv)
        {
            ...
            arv = StretchAudioChannels(output, channels, temp);

            return complete;
        }

        /// <summary>
        /// Stretch mono audio into a buffer with more channels
        /// </summary>
        /// <param name="output">The output buffer to write the stretched audio samples into. Its length should be a multiple of <paramref name="channels"/>.</param>
        /// <param name="channels">The number of audio channels in the output buffer.</param>
        /// <param name="monoAudio">The input buffer containing mono audio samples to be stretched across the output channels.</param>
        /// <returns>The average rectified value</returns>
        internal static float StretchAudioChannels([NotNull] float[] output, int channels, [NotNull] float[] monoAudio)
        {
            // Step through samples, stretching them (i.e. play mono input in all output channels)
            float accumulator = 0;
            var sampleIndex = 0;
            for (var i = 0; i < output.Length; i += channels)
            {
                ...
                var sample = monoAudio[sampleIndex++];
            ...
            return accumulator / sampleIndex;
        }
    }
}
