...
namespace Sim.Jobs.UI
{
    ...
    /// </summary>
    public class JobRibbonController : MonoBehaviour
    {
        [Header("Settings")]
        ...
        private void Subscribe()
        {
            ...
            c.JobOffered += OnJobStateUpdated;
            c.JobStepAdvanced += OnJobStateUpdated;
            c.JobFinished += OnJobFinished;
            _isSubscribed = true;
            
            // Initial sync if a job is already active
            var activeJob = c.States.Values.FirstOrDefault(s => s.Status == JobStatus.Active);
            if (activeJob != null) OnJobStateUpdated(activeJob);
        }
        ...
        private void OnJobStateUpdated(JobClientState state)
        {
        ...
        private void UpdateTarget(string targetId)
        {
            ...
            if (JobPoint.ByPointId.TryGetValue(targetId, out var point))
            {
                EnsureRibbon();
        ...
        private void EnsureRibbon()
        {
            ...
            {
                Debug.LogWarning("[JobRibbonController] Ribbon prefab is missing!");
            }
        }
