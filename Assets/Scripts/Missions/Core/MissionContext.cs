...
namespace Sim.Missions {
    ...
    public class MissionContext {
        private readonly Dictionary<string, IMissionTarget> _targets = new Dictionary<string, IMissionTarget>();
        ...
        public void SetTarget(string key, IMissionTarget target) {
            if (string.IsNullOrEmpty(key)) return;
            if (target == null) _targets.Remove(key);
            else _targets[key] = target;
        }

        public IMissionTarget TargetByKey(string key) {
            if (string.IsNullOrEmpty(key)) key = PrimaryKey;
            return _targets.TryGetValue(key, out var t) ? t : null;
        }

        public IMissionTarget TargetByKey(MissionTargetKey key) => TargetByKey(key.ToKey());

        public void SetTarget(MissionTargetKey key, IMissionTarget target) => SetTarget(key.ToKey(), target);

        public bool HasTarget(string key) => _targets.ContainsKey(key);
        public IEnumerable<KeyValuePair<string, IMissionTarget>> Targets => _targets;

        public IMissionTarget primaryTarget {
            get => TargetByKey(PrimaryKey);
            set => SetTarget(PrimaryKey, value);
        }

        public IMissionTarget secondaryTarget {
